  ...
  <MtgaDialog v-model:open="openModel" max-width="max-w-3xl" @close="handleClose">
    <template #header>
      <div class="flex items-start justify-between gap-3">
        <div class="space-y-1">
          <h3 class="text-lg font-semibold text-white">主题配置</h3>
          <p class="text-xs text-slate-400">保存后会立即应用到当前界面，并持久化到本地。</p>
        </div>
        <button
          class="btn btn-xs h-7 rounded-lg border-slate-700 bg-slate-800 px-3 font-medium text-slate-400 hover:border-rose-500 hover:bg-rose-500/10 hover:text-rose-400 transition-all"
          @click="handleReset"
        >
          重置
        </button>
    ...
    <div class="px-6 py-5 space-y-5">
      <div class="grid gap-3 md:grid-cols-2">
        <div
          ...
          :key="field.key"
          class="group flex items-center gap-3 rounded-xl border border-slate-700/60 bg-slate-900/50 p-2 transition-all hover:border-cyan-500/50 hover:bg-slate-800/80 hover:shadow-sm"
        >
          <!-- 标签 -->
          <span
            class="text-sm font-semibold text-slate-500 min-w-[80px] pl-1 group-hover:text-cyan-600 transition-colors"
          >
            {{ field.label }}
          </span>
          ...
          <div class="flex flex-1 items-center justify-end gap-2">
            <!-- 颜色预览和选择器 -->
            <div
              class="relative h-7 w-9 shrink-0 overflow-hidden rounded-md border border-slate-700 shadow-sm transition-transform active:scale-95"
            >
              <input
                type="color"
      ...
      <div
        class="rounded-2xl border border-slate-700/60 bg-slate-900/50 p-4 transition-all hover:border-cyan-500/50 hover:bg-slate-800/80 hover:shadow-[0_0_15px_rgba(6,182,212,0.15)]"
      >
      ...
      <div
        class="rounded-2xl border border-slate-700/60 bg-slate-900/50 p-4 space-y-3 transition-all hover:border-cyan-500/50 hover:bg-slate-800/80 hover:shadow-[0_0_15px_rgba(6,182,212,0.15)]"
      >
        <div class="flex items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-2">
            <button
              class="btn btn-xs h-7 rounded-lg border-slate-700 bg-slate-800 px-3 font-medium text-slate-300 hover:border-cyan-500 hover:bg-cyan-900/30 hover:text-cyan-400 transition-all"
              @click="openBackgroundPicker"
            >
              上传图片
            </button>
            <button
              class="btn btn-xs h-7 rounded-lg border-slate-700 bg-slate-800 px-3 font-medium text-slate-300 hover:border-rose-500 hover:bg-rose-900/30 hover:text-rose-400 transition-all"
        ...
        <div
          class="relative h-28 overflow-hidden rounded-xl border border-dashed border-slate-600 bg-slate-900/30 flex items-center justify-center"
        >
          <div
            v-if="themeDraft.backgroundImage"
            class="absolute inset-0 bg-cover bg-center bg-no-repeat"
            :style="{ backgroundImage: `url('${themeDraft.backgroundImage}')` }"
          ></div>
          <div v-else class="flex flex-col items-center justify-center text-slate-500">
            <svg
            ...
            <span class="text-xs">未设置背景图片，将使用默认渐变背景</span>
          </div>
        </div>
      </div>
