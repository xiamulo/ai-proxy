...
<template>
  ...
  <div role="tablist" class="mt-4 flex flex-wrap gap-3 border-b border-slate-700/70 pb-2">
    <button
      v-for="tab in tabs"
      :key="tab.key"
      role="tab"
      type="button"
      class="cursor-pointer px-3 py-2 text-sm font-medium text-slate-400 transition-colors duration-150"
      :class="
        activeTab === tab.key
          ? 'border-b-2 border-cyan-500 text-white'
          : 'border-b-2 border-transparent hover:text-slate-200'
      "
